...
| A live student session sees sections of offerings they are NOT enrolled in | `tests/app-schema/test-isolation.mjs` | Found 2026-08-17, the first live-suite run since the test cadet's rotation: "student sees only sections in offerings they are enrolled in — **saw 21**". Nothing in that day's changes touches sections or RLS, so this predates it and its age is unknown — the suite was blind while the credential was stale. Same family as P3.9's offering-wide-visibility question; wants a read of `sections`' RLS policy against what the test cadet is actually enrolled in |
