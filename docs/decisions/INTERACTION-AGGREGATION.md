...
The consumer already exists in `site/app/faculty/interactions.html` (`renderAggregate` → `aggregateHTML`);
wiring is additive:
...
- **Numbers:** reuse `summarizeReports()` (`site/app/js/faculty-interactions.js`) — single source of truth
