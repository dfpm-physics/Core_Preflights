...
1. **Migration `014_interaction_analysis.sql` is applied.** It creates the `interaction_analysis`
   table this skill writes. You (Claude) have **no DDL rights** — if `pull`/`status` errors with
...
ones into known buckets (taxonomy ids in CLAUDE.md / the contract), note section concentration and
