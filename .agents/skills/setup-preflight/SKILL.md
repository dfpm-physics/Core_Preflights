...
```bash
ls ~/.Codex/skills/preflight-analyze/config.json 2>/dev/null && echo "EXISTS" || echo "NOT_FOUND"
```
...
```bash
mkdir -p ~/.Codex/skills/preflight-analyze
```

Then write the config using a here-doc so special characters in the key are handled safely:

```bash
cat > ~/.Codex/skills/preflight-analyze/config.json << 'ENDOFCONFIG'
{
...
> ✓ Config written to `~/.Codex/skills/preflight-analyze/config.json`
...
const cfg = JSON.parse(fs.readFileSync(path.join(process.env.HOME, '.Codex/skills/preflight-analyze/config.json'), 'utf8'));
...
> ✅ **Setup complete!**
>
> Your config is saved at `~/.Codex/skills/preflight-analyze/config.json`
>
> ---
>
...
- **Permission denied writing config**: If `mkdir` or writing the file fails, suggest
  running `mkdir -p ~/.Codex/skills/preflight-analyze` manually and checking permissions.
