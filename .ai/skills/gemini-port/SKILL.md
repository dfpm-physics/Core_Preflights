...
**It is a fallback in the routing sense, and the framing has moved on.** The router page still tells a cadet to keep trying Claude, and it is the only surface that does: the build's own **BACKUP VERSION banner was removed on 2026-08-21**, on the course director's call, because it told cadets to prefer the version that is timing out on them. *(This sentence said “both the router page and the build's own banner say so on their face”, which was true until that day. The router copy was left alone deliberately — changing cadet-facing wording there is a separate decision.)* There are now **two doors** into the backup from the artifact, both cadet-clicked and neither automatic:
