...
> **A FOURTH shape arrived on 2026-09-22, and it is the opposite problem: a source AHEAD of the porter.** PHYS 110 lesson 19 is the first artifact built carrying the **2026-08-21** fix set (`docs/operations/TUTOR-BEHAVIOR-PARITY.md` §5) — request deadline, 5xx and empty-response ladder walks, the finish bar, the three-state lz-string loader. This tool exists to **add** those. A source that already has them makes every one of those transforms refuse, and the fix is not the usual one: there is no layout to loosen, because both files are *correct* and simply disagree about which of them performs the work.
>
> Four anchors were made tolerant that day and are in the tool now — constants, `callTutor`, the lz-string hook, `lzReady`. **Three collisions remain unresolved**: the finish bar, `rawCall` and `errorMessage`, where the porter and the artifact each carry a full implementation of the same feature (`continueInStudy` + `stampSubmitted` against `keepTalking` + an inline confirm). Reconciling those through byte anchors means merging two designs in JSX, which is the failure shape PROJECT.md's table warns about — parses, renders, serves, then throws on a cadet. **So lesson 19 has no Gemini build**, and the lessons page falls back to its Claude launch button.
>
> **Two traps inside that work, both paid for already.** The lz-hook replacement declares `LZ_ATTEMPTS` and `LZ_ATTEMPT_MS`, which a §5.6 source *also* declares — leaving the source's pair behind emits two `const`s in one scope, a SyntaxError thrown before React mounts, i.e. a blank page rather than a failed check. And a prefix group written to eat those constants must be **gated on there being one to eat**: the first version also matched an empty run and swallowed the blank line above the hook on all 50 unpatched sources, turning `unchanged` into `WOULD WRITE` fleet-wide. **After any anchor change, run `--all-courses --all` and confirm a recently-ported build still reports `unchanged`** — lesson 18 is the current witness. Lessons 10–17 report `WOULD WRITE` against the original tool too; that is `patch_tutor_diagnostics.py` having patched them in place (parity §2.13), not your change.
>
> The real fix is for the porter to **inherit** the 2026-08-21 set the way it already inherits 2026-08-20 — detect what the source carries and skip that transform. That is a deliberate piece of work on a tool serving 50 artifacts, and it has not been done.
...
**A 429 moves down the ladder instead of ending the session — but only after the body is read.** Gemini returns 429 for both the per-minute and the per-day limit, and the transport marks that model spent and steps to the next rung, which carries an independent quota. Spent models stay spent for the session, so the extension phase cannot re-burn one already known exhausted. Only a fully walked ladder raises `quota`, and only then does it wait — once, 45s, which is the measured recovery plus margin.

**Which limit it was is read from the QUOTA NAME, never from the delay.** *(Rewritten 2026-08-26 against 107 measured requests; see [`docs/operations/TUTOR-BEHAVIOR-PARITY.md`](../../../docs/operations/TUTOR-BEHAVIOR-PARITY.md) §2.10.)* A named refusal carries `GenerateRequestsPer**Minute**PerProjectPerModel-FreeTier` or `…Per**Day**…` in its `QuotaFailure`, which is unambiguous. **The delay is not**: the measured per-minute wall named 31s and the measured per-day wall named 44s, so "long delay ⇒ daily" is simply wrong, and `RetryInfo` was separately caught over-stating (57s on a wall that cleared in 10) and under-stating (31s on one that took 41s). It is therefore **clamped in both directions and never obeyed**.

**When Google names nothing — which is the common case in live data — the transport says so and does not guess.** All 781 quota refusals logged on 2026-08-26 carried a bare `"Resource has been exhausted (e.g. check quota)."` with no name, no limit and no delay, on models that had received a single request against caps of 15/minute and 500/day. A daily claim therefore additionally requires the client's own ledger to show ~80% of that model's daily cap actually sent; without it the cadet is told Google is busy, not that their allowance is gone.
