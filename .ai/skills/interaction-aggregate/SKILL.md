...
> [`INTERACTION-AGGREGATION.md`](../../../docs/decisions/INTERACTION-AGGREGATION.md) spec describes and that
...
1. **Migration `014_interaction_analysis.sql` is applied.** It creates the `interaction_analysis`
   table this skill writes. The AI operator has **no DDL rights** — if `pull`/`status` errors with
...
> **The faculty rollup does NOT display this yet.** Wiring `site/app/faculty/interactions.html` to read
