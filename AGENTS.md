...
- **Analysis**: `/preflight-analyze` Codex skill (see `.Codex/skills/preflight-analyze/`)
...
| `interaction-submit.html` | Receives a Codex artifact's compressed report and saves it per student |
...
| `interactions` | Lesson interactions (Codex artifacts); `id` is a slug like `lesson-02-charge`; has `artifact_url`, `is_published` |
...
## Lesson Interactions (Codex Artifacts)

*Added 2026-06-11 by Matthew Recker — see `CHANGELOG.md`.*

A second path alongside assignments. A **lesson interaction** is a Codex artifact (an
interactive lesson hosted on Codex.ai). Students launch it, work through it, and the
...
2. A student opens `interactions.html`, clicks **Launch**, and the artifact opens on Codex.ai.
...
coordination point between the Codex.ai artifact and this repo. The **full, frozen contract**
...
**Prefill links:** a Codex artifact can hand the director a one-click link that opens
...
1. `cp .Codex/skills/preflight-analyze/config.json.template ~/.Codex/skills/preflight-analyze/config.json`
...
- **Always update `CHANGELOG.md`** when shipping any feature, fix, or documentation change — include date (YYYY-MM-DD), your name, and what/why. For Codex-authored changes, attribute to the instructor who requested it (e.g. "Casey Pellizzari via Codex").
